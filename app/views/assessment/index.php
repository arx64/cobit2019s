    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0"><i class="bi bi-person-lines-fill me-2"></i>Pilih Responden</h5>
            </div>
            <div class="card-body">
                <div class="process-selector d-flex justify-content-evenly flex-wrap gap-2">
                    <?php foreach ($respondents as $respondent): ?>
                        <a href="index.php?page=data-penilaian&process=<?php echo $selectedProcessId; ?><?php echo $respondent['id'] ? '&respondent_id=' . $respondent['id'] : ''; ?><?php echo $selectedDate ? '&date=' . urlencode($selectedDate) : ''; ?>"
                            class="process-option <?php echo $selectedRespondentId == $respondent['id'] ? 'active' : ''; ?>">
                            <div class="process-code"><?php echo htmlspecialchars($respondent['name']); ?></div>
                            <div class="process-name"><?php echo htmlspecialchars($respondent['position']); ?> (<?php echo $respondent['category'] === 'operator_sistem' ? 'Operator Sistem' : 'Perangkat Desa'; ?>)</div>
                        </a>
                    <?php endforeach; ?>
                </div>
                <?php if (!$selectedRespondent): ?>
                    <div class="alert alert-info mt-3 mb-0">
                        Pilih responden terlebih dahulu untuk mulai mengisi penilaian.
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                <div>
                    ...
                    <small class="text-muted">Pilih tanggal untuk melihat aktivitas penilaian pada tanggal tertentu.</small>
                </div>
                <form method="GET" class="d-flex align-items-center gap-2 flex-wrap">
                    <input type="hidden" name="page" value="data-penilaian">
                    <input type="hidden" name="process" value="<?php echo $selectedProcessId; ?>">
                    <input type="hidden" name="respondent_id" value="<?php echo $selectedRespondentId; ?>">
                    <label class="mb-0" for="dateFilter">Tanggal:</label>
                    <input
                        type="date"
                        id="dateFilter"
                        name="date"
            ...
            <div class="card-body">
                <div class="process-selector d-flex justify-content-evenly flex-wrap gap-2">
                    <?php foreach ($processes as $process): ?>
                        <a href="index.php?page=data-penilaian&process=<?php echo $process['id']; ?><?php echo $selectedRespondentId ? '&respondent_id=' . $selectedRespondentId : ''; ?><?php echo $selectedDate ? '&date=' . urlencode($selectedDate) : ''; ?>"
...
<div class="row">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <small class="text-muted">
                        <?php if ($selectedRespondent): ?>
                            Responden: <?php echo htmlspecialchars($selectedRespondent['name']); ?> (<?php echo htmlspecialchars($selectedRespondent['position']); ?>)
                        <?php else: ?>
                            Pilih responden terlebih dahulu untuk memulai penilaian.
                        <?php endif; ?>
                    </small>
                </div>
            ...
            <div class="card-body">
                <?php if (!$selectedRespondent): ?>
                    <div class="alert alert-warning">Silakan pilih responden terlebih dahulu.</div>
